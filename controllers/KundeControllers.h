...
#include "../business/ValidationModule.h"
...
class KundeControllers : public drogon::HttpController<KundeControllers>
{
  public:
    ...
    ADD_METHOD_TO(KundeControllers::GetAllValidation , "/Kunde/{1}" , Get , "LoginFilter");
    METHOD_LIST_END
    ...
    void createOne(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback, Kunde &&KUNDEN) const;
    ...
    void GetAllValidation(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback  , std::string Valid_CD) const; 
};
